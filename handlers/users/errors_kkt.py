...
async def show_instructions_buttons(message: Message):
    await message.answer('Выберите ошибку по номеру или по модели ККТ', reply_markup=error_type)
...
@dp.callback_query_handler(text='back_to_type_error')
async def back(call: CallbackQuery):
    await call.answer(cache_time=60)
    await call.message.delete()
    await call.message.answer('Выберите ошибку по номеру или по модели ККТ',
                              reply_markup=error_type)  # здесь это реализовано через тупой костыль "удалить
